...
from axiomm import axiom, app, db, lang, queue, tg, yt
from axiomm.helpers import admin_check, buttons, can_manage_vc
...
async def _controls(_, query: types.CallbackQuery):
    ...
    if action == "pause":
        ...
        return await axiom.play_media(chat_id, msg, media)

    elif action == "replay":
        media = queue.get_current(chat_id)
        media.user = user
        await axiom.replay(chat_id)
        status = query.lang["replayed"]
        reply = query.lang["play_replayed"].format(user)

    elif action == "stop":
        await axiom.stop(chat_id)
