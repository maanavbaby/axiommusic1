...
from axiomm import axiom, app, db, lang, queue
from axiomm.helpers import can_manage_vc


@app.on_message(filters.command(["seek", "seekback", "aage"]) & filters.group & ~app.bl_users)
@lang.language()
@can_manage_vc
async def _seek(_, m: types.Message):
    ...
    await axiom.play_media(m.chat.id, sent, media, start_from)
