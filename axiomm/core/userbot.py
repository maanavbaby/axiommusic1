...
from axiomm import config, logger
...
class Userbot(Client):
    ...
    async def boot_client(self, num: int, ub: Client):
        ...
        await client.start()
        try:
            await client.send_message(config.LOGGER_ID, "<blockquote>✧ <b> ᴧxɪσϻ's ϻυsɪᴄ ʙσᴛ ᴧssɪsᴛᴧηᴛ sᴛᴧʀᴛєᴅ sυᴄᴄєssғυʟʟʏ</b> 😁💝</blockquote>")
        except Exception:
        ...
        try:
            await ub.join_chat("manavkiduniya")
            await ub.join_chat("axiombots")
        except Exception:
            pass
